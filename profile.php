		...
		if($extension == 'png' || $extension == 'jpg' || $extension == 'jpeg') {
			if(!$_FILES['photo_file']['error']) {
				...
				if($valid_file) {
                    ...
                    $userid = $user->id;
                    $time = time();
                    print "$userid, $new_file_name, $time";
//                    INSERT INTO `uploaded_photos` (`id`, `user_id`, `path`, `is_instagram`, `time`) VALUES (NULL, '121', '5e3af0abea0dd17b34abd3cd865d05beresized-cropped-me.jpg', '0', '1231313');
                    $query = "INSERT INTO uploaded_photos (user_id,path,time, is_instagram) VALUES ('$userid','$new_file_name','$time', '0')";
                    print "<br />" . $query . ": query 1;";
                    $query2 = "UPDATE users SET uploaded_photos=uploaded_photos+1 WHERE id='$userid'";
                    print "<br />$query2: query2";
				$db->query($query);
					$db->query("UPDATE users SET uploaded_photos=uploaded_photos+1 WHERE id='$userid'");
                    print "<h1>" . mysqli_error($db)  . "fck</h1>";
				}
			}
			else {
